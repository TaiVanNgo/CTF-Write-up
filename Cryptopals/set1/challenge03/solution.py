...
def attack_single_byte_xor(cipher_text):
    ...
    for i in range (2**8):
        ...
        key_stream = candidate_key * len(cipher_text) #make a candidate_key have the same length with the encrypted text
        candidate_message = bxor(cipher_text, key_stream)
        number_of_letters = sum([x in ascii_text_list for x in candidate_message])
