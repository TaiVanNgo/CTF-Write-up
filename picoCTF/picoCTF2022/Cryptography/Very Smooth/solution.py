from gmpy2 import gcd, next_prime
from Crypto.Util.number import long_to_bytes


# Source that I check to build thí code: https://www.youtube.com/watch?v=bI_Opi4KdXo (the lecture of Pollard p-1)
# Reference of code for this algorithm: https://www.geeksforgeeks.org/pollard-p-1-algorithm/


n = 
c = 
e = 0x10001


def pollard(n):
    a = 2    # base
    i = 2    # exponent

    while True:
        a = pow(a, i, n)
        p = gcd(a - 1, n)

        if (p != 1 and p != n):
            return (p, n//p)

        i = next_prime(i)


(p, q) = pollard(n)

phi_n = (p-1)*(q-1)


d = pow(e, -1, phi_n)
m = pow(c, d, n)

print(long_to_bytes(m).decode('utf-8'))
