from math import isqrt
from gmpy2 import lcm

x = 
n = 
c = 

# To decrypt the message, we need the private key (d)
# d = e^(-1) mod phi_n) -> or pow(e, -1, phi_n)
# we have n but do not have phi_n
# phi_n = (p-1)*(q-1)
# --> We need to find p and q

# Given the sum and product of p and q, which is x and n
# x = p + q => p = x - q
# n = p * q => n = (x - q) * q => n = xq - q^2 => q^2 -xq + n
# So we can find the result of this quadratic equation. -> p and q are the result from the equation

# then we can decrypt the flag by
# m = pow(c, d, n)

e = 65537
# Find p and q by solving this equation q^2 -xq + n
# -> a = 1, b = -x, c = n
delta = x**2 - 4 * n
sqrt_delta = isqrt(delta)  # find square root of delta

p = (x + sqrt_delta) // 2
q = (x - sqrt_delta) // 2

phi_n = (p-1) * (q-1)

d = pow(e, -1, phi_n)  # private key

# find the message based on private key
m = pow(c, d, n)

print('FLAG: ', m.to_bytes((m.bit_length() + 7) // 8, 'big'))
